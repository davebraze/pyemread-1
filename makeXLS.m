function [wLabels tStamps] = makeXLS(TextGrid)

[segs,labs]=ReadPraatTier(TextGrid, 'word');
srate = 250;

tStamps = segs(1,1): 1/srate: segs(end,2);
idx = [];
for i = 1:length(tStamps)
    tmp=find((segs(:,1)<=tStamps(i) & segs(:,2)>=tStamps(i))==1);
    idx(i) = tmp(1);
end

wLabels = labs(idx);

fid = fopen([TextGrid(1:end-9) '_ETime.csv'], 'w');
for i = 1:length(wLabels)
    fprintf(fid, '%s,%s\n', wLabels{i}, num2str(tStamps(i)));
end
fclose(fid);
...
try,
    ...
    fclose(fid);
    if size(lines) < 2
        fid = fopen(fName,'r','n','UTF-8'); % otherwise, read UTF-8
        lines = {};
        while 1,
            line = fgetl(fid);
            if ~ischar(line), break; end;
